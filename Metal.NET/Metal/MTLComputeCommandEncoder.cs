...
/// <summary>Encodes computation dispatch commands for a single compute pass into a command buffer.</summary>
public class MTLComputeCommandEncoder(nint nativePtr, NativeObjectOwnership ownership) : MTLCommandEncoder(nativePtr, ownership), INativeObject<MTLComputeCommandEncoder>
{
    ...
    /// <summary>The dispatch type to use when submitting compute work to the GPU.</summary>
    ...
    /// <summary>Configures the compute encoder with a pipeline state for subsequent kernel calls.</summary>
    ...
    /// <summary>Binds a buffer to the buffer argument table, allowing compute kernels to access its data on the GPU.</summary>
    ...
    /// <summary>Changes where the data begins in a buffer already bound to the buffer argument table.</summary>
    ...
    /// <summary>Copies data directly to the GPU to populate an entry in the buffer argument table.</summary>
    ...
    /// <summary>Binds a texture to the texture argument table, allowing compute kernels to access its data on the GPU.</summary>
    ...
    /// <summary>Encodes a texture sampler, allowing compute kernels to use it for sampling textures on the GPU.</summary>
    ...
    /// <summary>Binds a visible function table to the buffer argument table, allowing you to call its functions on the GPU.</summary>
    ...
    /// <summary>Binds an acceleration structure to the buffer argument table, allowing functions to access it on the GPU.</summary>
    ...
    /// <summary>Binds an intersection function table to the buffer argument table, making it callable in your Metal shaders.</summary>
    ...
    /// <summary>Ensures kernel calls that the system encodes in subsequent commands have access to a resource.</summary>
    public void UseResource(MTLResource resource, MTLResourceUsage usage)
    {
        ObjectiveC.MsgSend(NativePtr, MTLComputeCommandEncoderBindings.UseResource, resource.NativePtr, (nuint)usage);
    }

    /// <summary>Ensures kernel calls that the system encodes in subsequent commands have access to multiple resources.</summary>
    ...
    /// <summary>Ensures the shaders in the render pass’s subsequent draw commands have access to all of the resources you allocate from a heap.</summary>
    public void UseHeap(MTLHeap heap)
    {
        ObjectiveC.MsgSend(NativePtr, MTLComputeCommandEncoderBindings.UseHeap, heap.NativePtr);
    }

    /// <summary>Ensures the shaders in the render pass’s subsequent draw commands have access to all of the resources you allocate from multiple heaps.</summary>
    ...
    /// <summary>Configures the size of a block of threadgroup memory.</summary>
    ...
    /// <summary>Sets the size, in pixels, of imageblock data in tile memory.</summary>
    ...
    /// <summary>Sets the dimensions over the thread grid of how your compute kernel receives stage-in arguments.</summary>
    ...
    /// <summary>Encodes a compute command using an arbitrarily sized grid.</summary>
    ...
    /// <summary>Encodes a compute dispatch command using a grid aligned to threadgroup boundaries.</summary>
    ...
    /// <summary>Encodes an instruction to run commands from an indirect buffer.</summary>
    ...
    /// <summary>Encodes a command that instructs the GPU to pause the compute pass until another pass updates a fence.</summary>
    public void WaitForFence(MTLFence fence)
    {
        ObjectiveC.MsgSend(NativePtr, MTLComputeCommandEncoderBindings.WaitForFence, fence.NativePtr);
    }

    /// <summary>Encodes a command that instructs the GPU to update a fence after the compute pass completes.</summary>
    public void UpdateFence(MTLFence fence)
    {
        ObjectiveC.MsgSend(NativePtr, MTLComputeCommandEncoderBindings.UpdateFence, fence.NativePtr);
    }

    /// <summary>Creates a memory barrier that enforces the order of write and read operations for specific resource types.</summary>
    ...
    /// <summary>Encodes a command to sample hardware counters, providing performance information.</summary>
