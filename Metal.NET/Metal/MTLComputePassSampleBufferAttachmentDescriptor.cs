...
/// <summary>A configuration that instructs the GPU where to store counter data from the beginning and end of a compute pass.</summary>
public class MTLComputePassSampleBufferAttachmentDescriptor(nint nativePtr, NativeObjectOwnership ownership) : NSObject(nativePtr, ownership), INativeObject<MTLComputePassSampleBufferAttachmentDescriptor>
{
    ...
    /// <summary>A specialized memory buffer that the GPU uses to store its counter data during a compute pass.</summary>
    ...
    /// <summary>An index within a counter sample buffer that tells the GPU where to store counter data from the start of a compute pass.</summary>
    ...
    /// <summary>An index within a counter sample buffer that tells the GPU where to store counter data from the end of a compute pass.</summary>
