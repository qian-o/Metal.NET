...
/// <summary>A configuration that customizes the behavior for a new command buffer.</summary>
public class MTLCommandBufferDescriptor(nint nativePtr, NativeObjectOwnership ownership) : NSObject(nativePtr, ownership), INativeObject<MTLCommandBufferDescriptor>
{
    ...
    /// <summary>The shader logging configuration that the command buffer uses.</summary>
    ...
    /// <summary>A Boolean value that indicates whether the command buffer the descriptor creates maintains strong references to the resources it uses.</summary>
    ...
    /// <summary>The reporting configuration that indicates which information the GPU driver stores in a command buffer’s error property.</summary>
