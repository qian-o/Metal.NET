...
class CSharpEmitter(string outputDir, GeneratorContext context, TypeMapper typeMapper)
{
    ...
    void GenerateClass(ClassDef classDef, HashSet<string> inheritedProperties, List<FreeFunctionDef> freeFunctions)
    {
        ...
        sb.AppendLine($"public {partialKeyword}class {csClassName}(nint nativePtr, bool owned) : {baseClass}(nativePtr, owned)");
        sb.AppendLine("{");
    ...
    void EmitMethod(StringBuilder sb, MethodInfo method, string csClassName, string ns, SortedDictionary<string, string> selectors, HashSet<string> methodsWithZeroParamProperty)
    {
        ...
        bool isOwned = TypeMapper.IsOwnershipTransferMethod(cppName);
        ...
        if (isVoid)
        {
            ...
            {
                sb.AppendLine();
                sb.AppendLine($"{indent}error = errorPtr is not 0 ? new(errorPtr, false) : null;");
            }
        }
        else if (nullable)
        {
            string ownedFlag = isOwned ? "true" : "false";
            if (hasOutError)
            {
                ...
                sb.AppendLine($"{indent}error = errorPtr is not 0 ? new(errorPtr, false) : null;");
                sb.AppendLine();
                sb.AppendLine($"{indent}return nativePtr is not 0 ? new(nativePtr, {ownedFlag}) : null;");
            }
            else
            {
                sb.AppendLine($"{indent}nint nativePtr = ObjectiveCRuntime.MsgSendPtr({argsStr});");
    ...
    void EmitFreeFunction(StringBuilder sb, FreeFunctionDef func, string cppNamespace)
    {
        ...
        if (nullable)
        {
            ...
            sb.AppendLine();
            sb.AppendLine("        return nativePtr is not 0 ? new(nativePtr, true) : null;");
            sb.AppendLine("    }");
        }
